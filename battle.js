/* ═══════════════════════════════════════════════════════════════
   ...
   Battle multijugador STEM en tiempo real.
   ...
   3. Battle → 5 rondas, 15s por pregunta
...
const Battle = (() => {
  ...
  ──────────────────────────────────────────────────────────── */
  const SUPABASE_ACTIVE = true; // ✅ Supabase conectado
  ...
  const QUESTIONS = {
    fis: [
      { q:'¿Cuál es la fórmula del Movimiento Rectilíneo Uniforme?', opts:['x = v·t','F = m·a','E = mc²','PV = nRT'], ans:0, xp:10 },
      { q:'En caída libre, ¿qué valor tiene g en la Tierra?', opts:['9.8 m/s²','10 m/s²','8.9 m/s²','7.8 m/s²'], ans:0, xp:10 },
      { q:'¿De qué depende el período de un péndulo simple?', opts:['Solo de su longitud','De su masa','De la amplitud','De la densidad'], ans:0, xp:15 },
      { q:'La Ley de Hooke establece que F es proporcional a:', opts:['La deformación x','La masa','La velocidad','La aceleración'], ans:0, xp:10 },
      { q:'¿Cuánto mide la velocidad de la luz?', opts:['3×10⁸ m/s','3×10⁶ m/s','3×10¹⁰ m/s','3×10⁴ m/s'], ans:0, xp:15 },
      { q:'La Segunda Ley de Newton establece:', opts:['F = m·a','E = m·c²','V = I·R','P = m·v'], ans:0, xp:10 },
      { q:'¿Cuál es la unidad de fuerza en el SI?', opts:['Newton','Joule','Pascal','Watt'], ans:0, xp:8 },
      { q:'En el tiro parabólico, ¿qué componente de la velocidad es constante?', opts:['La horizontal','La vertical','Ambas','Ninguna'], ans:0, xp:15 },
      { q:'La energía cinética se calcula como:', opts:['½mv²','mgh','mv','Fd'], ans:0, xp:12 },
      { q:'¿Qué ley describe V = I·R?', opts:['Ley de Ohm','Ley de Faraday','Ley de Newton','Ley de Hooke'], ans:0, xp:10 },
    ],
    quim: [
      { q:'¿Cuántos electrones tiene el oxígeno?', opts:['8','6','10','16'], ans:0, xp:8 },
      { q:'El pH del agua pura es:', opts:['7','1','14','0'], ans:0, xp:8 },
      { q:'¿Qué indica un pH menor a 7?', opts:['Solución ácida','Solución básica','Solución neutra','Solución salina'], ans:0, xp:10 },
      { q:'La fórmula de los gases ideales es:', opts:['PV = nRT','E = mc²','F = ma','V = IR'], ans:0, xp:12 },
      { q:'¿Cuál es el número atómico del hidrógeno?', opts:['1','2','8','6'], ans:0, xp:8 },
      { q:'¿Qué tipo de enlace comparten los átomos de NaCl?', opts:['Iónico','Covalente','Metálico','Van der Waals'], ans:0, xp:15 },
      { q:'¿Cuántos protones tiene el carbono?', opts:['6','8','12','4'], ans:0, xp:8 },
      { q:'El símbolo del oro en la tabla periódica es:', opts:['Au','Go','Or','Ag'], ans:0, xp:10 },
      { q:'Una reacción que libera calor se llama:', opts:['Exotérmica','Endotérmica','Catalítica','Redox'], ans:0, xp:12 },
      { q:'¿Cuál es el gas más abundante en la atmósfera?', opts:['Nitrógeno','Oxígeno','CO₂','Argón'], ans:0, xp:10 },
    ],
    mate: [
      { q:'¿Cuánto es √144?', opts:['12','11','13','14'], ans:0, xp:8 },
      { q:'¿Cuánto es 2³?', opts:['8','6','9','4'], ans:0, xp:8 },
      { q:'El área de un círculo es:', opts:['πr²','2πr','πd','r²'], ans:0, xp:10 },
      { q:'¿Cuánto es log₁₀(1000)?', opts:['3','2','4','10'], ans:0, xp:12 },
      { q:'La derivada de x² es:', opts:['2x','x²','2','x'], ans:0, xp:15 },
      { q:'¿Cuánto es sen(90°)?', opts:['1','0','√2/2','∞'], ans:0, xp:10 },
      { q:'¿Cuánto es 5! (factorial)?', opts:['120','60','24','100'], ans:0, xp:10 },
      { q:'La suma de ángulos de un triángulo es:', opts:['180°','360°','90°','270°'], ans:0, xp:8 },
      { q:'¿Cuánto es 15% de 200?', opts:['30','20','25','15'], ans:0, xp:8 },
      { q:'El teorema de Pitágoras establece: a²+b²=', opts:['c²','ab','2c','c³'], ans:0, xp:10 },
    ],
  };
  ...
  function createRoom(){
    ...
    // Save sala en localStorage (fallback) Y en Supabase
    const room = {
      id:       code,
      host_id:  state.me.id || code,
      host_name:   state.me.name,
      host_avatar: state.me.avatar,
      topic:    state.topic,
      rounds:   state.rounds,
      status:   'waiting',
      questions:[],
    ...
    // ── SUPABASE: crear sala en tiempo real ──
    if(window._supa){
      SupaSync.createBattle(room).then(data => {
        if(data) {
          // Suscribirse a cambios del rival
          state.channel = SupaSync.subscribeBattle(code, (updated) => {
            if(updated.guest_id && !state.opponent){
              state.opponent = {
                name:   updated.guest_name,
                avatar: updated.guest_avatar,
                elo:    1000,
              };
              setText('bt-opp-avatar', updated.guest_avatar);
              setText('bt-opp-name',   updated.guest_name);
              setText('bt-opp-ready',  '✓ Ready');
              const btn = document.getElementById('bt-start-btn');
              if(btn) btn.disabled = false;
              showToast('⚔️ ' + updated.guest_name + ' joined your room!', 3000);
            }
            // Refresh estado de la batalla
            if(updated.status === 'playing' && state.screen === 'waiting'){
              state.questions = updated.questions || buildQuestions(state.topic, state.rounds);
              startBattle();
            }
          });
        }
  ...
  function joinRoom(){
    ...
    if(code.length !== 6){ showToast('❌ El código debe tener 6 caracteres', 2500); return; }

    // ── SUPABASE: unirse a sala real ──
    if(window._supa){
      const battleData = await SupaSync.getBattle(code);
      if(!battleData){ showToast('❌ Room not found. Check the code.', 2500); return; }
      if(battleData.status !== 'waiting'){ showToast('⚠️ Room is already in game', 2500); return; }
      ...
      const ps   = JSON.parse(localStorage.getItem('yl_pstats')||'{}');
      state.me = {
        id:     ses.email || ses.guestId,
        name:   prof.name || ses.name || 'Guest',
        avatar: prof.avatar || ses.flag || '👤',
        elo:    ps.maxElo || 1000,
      };
      state.opponent = { name: battleData.host_name, avatar: battleData.host_avatar, elo:1000 };
      state.topic  = battleData.topic;
      state.rounds = battleData.rounds;

      // Refresh sala en Supabase
      await SupaSync.joinBattle(code, state.me);
      ...
      showToast('✅ Joined ' + state.opponent.name + "'s room", 3000);
      return;
    }

    // ── FALLBACK LOCAL ──
    const room = JSON.parse(localStorage.getItem('yl_room_'+code) || 'null');
    if(!room){ showToast('❌ Sala no encontrada. Verifica el código.', 2500); return; }
    if(room.status !== 'waiting'){ showToast('⚠️ La sala ya está en juego', 2500); return; }
    ...
    const prof = JSON.parse(localStorage.getItem('yl_profile')||'{}');
    const ps   = JSON.parse(localStorage.getItem('yl_pstats')||'{}');
    state.me = { name: prof.name||ses.name||'Tú', avatar: prof.avatar||ses.flag||'👤', elo: ps.maxElo||1000 };
  ...
  ──────────────────────────────────────────────────────────── */
  function nextRound(){
    if(state.currentRound >= state.rounds){ endBattle(); return; }
    ...
    const topicMap = { fis:'⚡ Physics', quim:'🧪 Chemistry', mate:'📐 Mathematics' };
    const topicKey = QUESTIONS.fis.includes(q) ? 'fis' : QUESTIONS.quim.includes(q) ? 'quim' : 'mate';
    ...
    if(rdot){ rdot.className = 'bt-rival-dot bt-rival-thinking'; }
  ...
  function updateTimerRing(val, max){
    ...
    ring.style.stroke = val <= 5 ? '#EF4444' : val <= 10 ? '#F59E0B' : '#5A5FE0';
    const numEl = document.getElementById('bt-timer-num');
    if(numEl) numEl.style.color = val <= 5 ? '#EF4444' : val <= 10 ? '#F59E0B' : '#fff';
  }
  ...
  function answer(idx){
    ...
    if(fb){
      if(correct){
        ...
        state.scores.me += pts;
        fb.innerHTML = `<span class="bt-fb-correct">✅ ¡Correct! +${pts} pts</span>`;
      } else {
        fb.innerHTML = `<span class="bt-fb-wrong">❌ Incorrect — era: ${q.opts[q.ans]}</span>`;
      }
    }
  ...
  function timeOut(){
    ...
    state.answered = true;
    const q = state.questions[state.currentRound];
    document.querySelectorAll('.bt-opt').forEach((b,i)=>{ b.disabled=true; if(i===q.ans) b.classList.add('bt-opt-correct'); });
    const fb = document.getElementById('bt-answer-feedback');
    if(fb) fb.innerHTML = `<span class="bt-fb-wrong">⏰ Time agotado — era: ${q.opts[q.ans]}</span>`;
    setTimeout(()=>{ state.currentRound++; nextRound(); }, 2000);
  }
  ...
  function simulateOpponentAnswer(q){
    ...
    setTimeout(()=>{
      if(!state.answered && state.timerVal > 0){
        ...
        if(rdot) rdot.className = 'bt-rival-dot ' + (correct ? 'bt-rival-correct' : 'bt-rival-wrong');
        if(rtxt) rtxt.textContent = state.opponent?.name + (correct ? ' ✅ respondió' : ' ❌ respondió');
        if(correct){
  ...
  function endBattle(){
    ...
    if(tie){
      ...
      setText('bt-result-title', '¡Draw!');
      setText('bt-result-sub',   'Los dos son excelentes — ¡revancha!');
    } else if(win){
      if(banner) banner.className = 'bt-result-banner bt-result-win';
      setText('bt-result-icon',  '🏆');
      setText('bt-result-title', 'You won!');
      setText('bt-result-sub',   'Amazing STEM performance!');
    } else {
      if(banner) banner.className = 'bt-result-banner bt-result-lose';
      setText('bt-result-icon',  '💪');
      setText('bt-result-title', '¡Defeat!');
      setText('bt-result-sub',   'Keep practicing, la revancha es tuya!');
    }
    ...
    setText('bt-xp-gained', `+${xpGained} XP ${win?'por victoria':'ganadas'}`);

    // Save en historial
    saveHistory({ win, tie, me: state.scores.me, opp: state.scores.opp, opponent: state.opponent, topic: state.topic, rounds: state.rounds, date: new Date().toISOString() });

    // Registrar en streak
    if(typeof Streak !== 'undefined'){
      Streak.register('practica', { label: `⚔️ Battle STEM ${win?'ganada':'completada'}`, correct: win });
    }
  ...
  function loadHistory(){
    ...
    if(!el) return;
    if(!h.length){ el.innerHTML = '<div class="bt-no-history">Aún no tienes batallas — ¡crea una sala!</div>'; return; }
    el.innerHTML = h.map(r => `
      <div class="bt-hist-row">
        <div class="bt-hist-result ${r.win?'bhr-win':r.tie?'bhr-tie':'bhr-lose'}">
          ${r.win?'Victory':r.tie?'Draw':'Defeat'}
        </div>
  ...
  function quickMatch(){
    ...
    setTimeout(()=>{
      if(btn){ btn.textContent = 'Buscar rival →'; btn.disabled = false; }
  ...
  function goLobby(){
    clearInterval(state.timerInterval);
    state = { ...state, screen:'lobby', roomCode:null, currentRound:0, scores:{me:0,opp:0}, questions:[], answered:false };
    showScreen('lobby');
    loadHistory();
  }
  ...
  function copyCode(){
    ...
    navigator.clipboard?.writeText(state.roomCode).then(()=>{
      ...
      setTimeout(()=>setText('bt-copy-btn', '📋 Copy code'), 2000);
    });
  }
  ...
  function updateOnlineCount(){
    ...
    setText('bt-online', nums[Math.floor(Math.random()*nums.length)] + ' jugadores online');
  }
  ...
  function showToast(msg, ms=3000){
    ...
    clearTimeout(el._bt); el._bt = setTimeout(()=>el.style.display='none', ms);
  }

  return { init, setTopic, setRounds, createRoom, joinRoom, quickMatch, startBattle, answer, cancelRoom, goLobby, rematch, copyCode };
